    ...
    public class LaneControl : UserControl
    {
        ...
        private bool ClearOfNextVehicle(Vehicle vehicleFromOtherLane)
        {
            ...
            if (nextVehicle != null)
            {
                ...
                // Specification requires vehicles changing to lane N-1 or lane N+1 must be able to travel at at least the current speed. If the vehicle from the other lane is allowed to overlap
                // then in theory it must change speed to match that of the next vehicle during the next tick. That is against the specification, so the vehicle from the other lane cannot overlap
                ...
                if (vehicleFromOtherLane.ProgressPixels + (vehicleFromOtherLaneProjectedStopppingDistancePixels * modifier) < backOfNextVehicle)
                {
        ...
        public bool AddVehicleToLane(Vehicle newVehicle)
        {
            ...
            if (lastVehicle != null)
            {
                ...
                int projectedDesiredStopppingDistancePixels = (int)Math.Round(MainForm.MetresToPixels(MainForm.StoppingDistance(newVehicle.DesiredSpeedMetresHour)));
                ...
                else
                {
                    ...
                    // Calculate the overlap allowed in pixels
                    int stoppingDistanceChangeByChangingSpeedsPixels = projectedDesiredStopppingDistancePixels - (int)Math.Round(MainForm.MetresToPixels(MainForm.StoppingDistance(lastVehicle.ActualSpeedMetresHour)));
